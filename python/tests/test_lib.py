...
from cartesian_tree import RPY, Frame, Pose, Position, Quaternion, Rotation
...
def test_pose_in_frame() -> None:
    base = Frame("base")
    frame_1 = base.add_child("frame1", Position(1, 1, 1), Quaternion(0, 0, 0, 1).to_rotation())
    frame_2 = base.add_child("frame2", Position(-2, 0, 0), RPY(0, 0, radians(90)).to_rotation())

    pose_in_frame1 = frame_1.add_pose(Position(0, 0, 0), Quaternion(0, 0, 0, 1).to_rotation())
...
def test_calibrate_frame() -> None:
    base = Frame("base")
    reference_frame = base.add_child("reference", Position(1, 1, 1), Quaternion(0, 0, 0, 1).to_rotation())
    reference_pose = reference_frame.add_pose(Position(1, 1, 1), Quaternion(0, 0, 0, 1).to_rotation())

    calibrated_frame = base.calibrate_child("calibrated", Position(0, 0, 0), RPY(0, 0, 0).to_rotation(), reference_pose)
...
def test_serialization() -> None:
    root = Frame("root")
    child1 = root.add_child("child1", Position(1, 0, 0), Quaternion(0, 0, 0, 1).to_rotation())
    child2 = child1.add_child("child2", Position(0, 1, 0), RPY(0, 0, radians(90)).to_rotation())
    child2.add_pose(Position(0, 0, 1), Quaternion(0, 0, 0, 1).to_rotation())

    json_str = root.to_json()

    default_root = Frame("root")
    default_child1 = default_root.add_child("child1", Position(2, 0, 0), Quaternion(0, 0, 0, 1).to_rotation())
    default_child2 = default_child1.add_child("child2", Position(0, 2, 0), RPY(0, 0, radians(90)).to_rotation())
